...
import tech.vvp.vvp.client.model.BradleyUkrModel;
...
public class BradleyUkrRenderer extends GeoEntityRenderer<BradleyUkrEntity> {
    ...
    public BradleyUkrRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new BradleyUkrModel());
        // this.addRenderLayer(new Yx100GlowLayer(this));
    }

    @Override
    public RenderType getRenderType(BradleyUkrEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, BradleyUkrEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(BradleyUkrEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, BradleyUkrEntity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        for (int i = 0; i < 41; i++) {
            ...
            if (name.equals("setka_turret")) {
                boolean hasMangal = animatable.getEntityData().get(BradleyUkrEntity.HAS_MANGAL);
                boolean hasFoliage = animatable.getEntityData().get(BradleyUkrEntity.HAS_FOLIAGE);
                bone.setHidden(!(hasMangal && hasFoliage));
            }

            if (name.equals("setka_body")) {
                boolean hasFoliage_body = animatable.getEntityData().get(BradleyUkrEntity.HAS_FOLIAGE_BODY);
