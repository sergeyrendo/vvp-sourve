...
import tech.vvp.vvp.client.model.SosnaModel;
...
import static tech.vvp.vvp.entity.vehicle.SosnaEntity.LOADED_MISSILE;

public class SosnaRenderer extends GeoEntityRenderer<SosnaEntity> {

    public SosnaRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new SosnaModel());
        // this.addRenderLayer(new Bmp2Layer(this));
    }

    @Override
    public RenderType getRenderType(SosnaEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, SosnaEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(SosnaEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, SosnaEntity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (name.equals("dulo") || name.equals("dulo2")) {
            ...
            if (animatable.getWeaponIndex(0) == 0) {
                int fire = animatable.getEntityData().get(SosnaEntity.FIRE_ANIM);
                if (fire > 1) {
                    float maxBack = 0.95f; // глубина отката
                    boolean leftBarrelFired = animatable.getEntityData().get(SosnaEntity.LAST_BARREL_LEFT);
