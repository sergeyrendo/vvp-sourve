...
public class Fab250Entity extends FastThrowableProjectile implements GeoEntity, ExplosiveProjectile {
    ...
    private static final DamageModifier DAMAGE_MODIFIER = DamageModifier.createDefaultModifier()
            .immuneTo(tech.vvp.vvp.init.ModEntities.FAB_250.get());

    private float explosionDamage = ExplosionConfigVVP.FAB_250_EXPLOSION_DAMAGE.get();
    private float explosionRadius = ExplosionConfigVVP.FAB_250_EXPLOSION_RADIUS.get().floatValue();
    ...
    public Fab250Entity(LivingEntity entity, Level level) {
        super(tech.vvp.vvp.init.ModEntities.FAB_250.get(), entity, level);
        this.noCulling = true;
    }

    public Fab250Entity(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this(tech.vvp.vvp.init.ModEntities.FAB_250.get(), level);
    }
