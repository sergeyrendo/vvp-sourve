...
public class Mi8MTVModel extends GeoModel<Mi8MTVEntity> {

    @Override
    public ResourceLocation getAnimationResource(Mi8MTVEntity entity) {
        return null;
//        return ModUtils.loc("animations/wheel_chair.animation.json");
    }

    @Override
    public ResourceLocation getModelResource(Mi8MTVEntity entity) {
        ...
        if (distance < 32) {
            return VVP.loc("geo/mi8_mtv3.geo.json");
        } else if (distance < 64) {
            return VVP.loc("geo/mi8_mtv3.geo.json");
        } else if (distance < 96) {
            return VVP.loc("geo/mi8_mtv3.geo.json");
    ...
    @Override
    public ResourceLocation getTextureResource(Mi8MTVEntity animatable) {
        int camoType = animatable.getEntityData().get(Mi8MTVEntity.CAMOUFLAGE_TYPE);
        switch (camoType) {
