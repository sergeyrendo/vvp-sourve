...
public class BrmRenderer extends GeoEntityRenderer<BrmEntity> {
    ...
    @Override
    public RenderType getRenderType(BrmEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, BrmEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(BrmEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, BrmEntity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (name.equals("dulo")) {
            ...
            if (animatable.getWeaponIndex(0) == 0) {
                int fire = animatable.getEntityData().get(BrmEntity.FIRE_ANIM); // или статический импорт FIRE_ANIM
                if (fire > 1) {
                    float maxBack = 0.70f; // глубина отката "назад" (подбери под модель)
