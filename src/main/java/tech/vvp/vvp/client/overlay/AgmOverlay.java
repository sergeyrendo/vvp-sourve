...
@OnlyIn(Dist.CLIENT)
public class AgmOverlay implements IGuiOverlay {
    public static final String ID = VVP.MOD_ID + "_agm_hud";
    ...
    public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        if (!(veh instanceof Mi28Entity mi28)) return;
        if (!(veh instanceof WeaponVehicleEntity weapon) || weapon.getWeaponIndex(0) == 0) return; // AGM-65 выбран
        ...
        Entity targetEntity = EntityFindUtil.findEntity(player.level(), mi28.getTargetUuid());
        List<Entity> entities = SeekTool.seekCustomSizeEntities(mi28, player.level(), 384, 20, 0.9, true);
        ...
        for (Entity e : entities) {
            ...
            boolean lockOn = mi28.locked && nearest;
            ...
            if (lockOn) {
                ...
                lerpLock = Mth.lerp(partialTick, lerpLock, 2 * mi28.lockTime);
