...
public class Fab500Entity extends FastThrowableProjectile implements GeoEntity {
    ...
    private static final DamageModifier DAMAGE_MODIFIER = DamageModifier.createDefaultModifier()
            .immuneTo(ModEntities.FAB_500.get());

    public Fab500Entity(EntityType<? extends Fab500Entity> type, Level world) {
        super(type, world);
        this.noCulling = true;
        this.explosionDamage = ExplosionConfigVVP.FAB_500_EXPLOSION_DAMAGE.get();
        this.explosionRadius = ExplosionConfigVVP.FAB_500_EXPLOSION_RADIUS.get().floatValue();
        this.gravity = 0.06f;
    }

    public Fab500Entity(LivingEntity entity, Level level) {
        super(ModEntities.FAB_500.get(), entity, level);
        this.noCulling = true;
        this.explosionDamage = ExplosionConfigVVP.FAB_500_EXPLOSION_DAMAGE.get();
        this.explosionRadius = ExplosionConfigVVP.FAB_500_EXPLOSION_RADIUS.get().floatValue();
        this.gravity = 0.06f;
    ...
    public Fab500Entity(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this(ModEntities.FAB_500.get(), level);
    }
