...
@OnlyIn(Dist.CLIENT)
public class Mi28_1HudOverlay implements IGuiOverlay {
    public static final String ID = VVP.MOD_ID + "_mi28_hud_1";
    ...
    public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        // 检查玩家是否在直升机实体中且是炮手位置
        if (player.getVehicle() instanceof Mi28_1Entity mi28_1 &&
                mi28_1.getNthEntity(1) == player) {
            ...
            if (mc.options.getCameraType() == CameraType.FIRST_PERSON) {
                // 第一人称视角渲染 - 始终使用机炮准星
                renderHelicopterHUD(guiGraphics, screenWidth, screenHeight, mi28_1, player, partialTick, true);
            } else if (mc.options.getCameraType() == CameraType.THIRD_PERSON_BACK) {
                // 第三人称视角渲染 - 根据武器类型使用不同准星
                renderHelicopterHUD(guiGraphics, screenWidth, screenHeight, mi28_1, player, partialTick, false);
            }
    ...
    private void renderHelicopterHUD(GuiGraphics guiGraphics, int screenWidth, int screenHeight,
                                     Mi28_1Entity mi28_1, Player player, float partialTick,
        ...
        // 1. 渲染准星（根据视角和武器类型选择）
        ResourceLocation crosshairTexture = getCrosshairTexture(mi28_1, isFirstPerson);
        preciseBlit(guiGraphics, crosshairTexture, xPos, yPos, 0, 0.0F, renderSize, renderSize, renderSize, renderSize);

        // 2. 渲染武器状态信息
        renderWeaponStatus(guiGraphics, screenWidth, screenHeight, mi28_1, player);
...
//        renderCompass(guiGraphics, screenWidth, mi28);
        ...
        // 5. 渲染炮塔角度指示器
        renderGunnerAngle(guiGraphics, player, mi28_1, xPos, yPos, renderSize, renderSize, partialTick);

        // 6. 渲染功率和速度信息
        renderPowerAndSpeedInfo(guiGraphics, screenWidth, screenHeight, mi28_1, partialTick);
     ...
     * 根据视角和武器类型获取准星纹理
     */
    private ResourceLocation getCrosshairTexture(Mi28_1Entity mi28_1, boolean isFirstPerson) {
        if (isFirstPerson) {
            // 第一人称始终使用机炮准星
            return CANNON_CROSSHAIR;
        } else {
            // 第三人称根据武器类型选择准星
            int weaponIndex = mi28_1.getWeaponIndex(1); // 获取炮手位当前武器索引
    ...
    private void renderWeaponStatus(GuiGraphics guiGraphics, int screenWidth, int screenHeight,
                                    Mi28_1Entity mi28_1, Player player) {
        // 获取武器热量值
        int heat = mi28_1.getEntityData().get(HEAT);
...
//    }
    ...
    private void renderPowerAndSpeedInfo(GuiGraphics guiGraphics, int screenWidth, int screenHeight,
                                         Mi28_1Entity mi28_1, float partialTick) {
        PoseStack poseStack = guiGraphics.pose();

        // 显示高度坐标（绿色）
        guiGraphics.drawString(Minecraft.getInstance().font,
                Component.literal(String.valueOf((int)mi28_1.getY())),
        ...
        double speed = Math.sqrt(
                mi28_1.getDeltaMovement().x * mi28_1.getDeltaMovement().x +
                        mi28_1.getDeltaMovement().y * mi28_1.getDeltaMovement().y +
                        mi28_1.getDeltaMovement().z * mi28_1.getDeltaMovement().z
        ) * 72;
