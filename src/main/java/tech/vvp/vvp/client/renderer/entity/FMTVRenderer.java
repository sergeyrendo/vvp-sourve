...
public class FMTVRenderer extends GeoEntityRenderer<FMTVEntity> {
    ...
    @Override
    public void preRender(PoseStack poseStack, FMTVEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(FMTVEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, FMTVEntity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (name.equals("tent")) {
            boolean has_tent = animatable.getEntityData().get(FMTVEntity.HAS_TENT);
            bone.setHidden(!has_tent);
        }
    ...
    @Override
    protected float getDeathMaxRotation(FMTVEntity entityLivingBaseIn) {
        return 0.0F;
    }
}
